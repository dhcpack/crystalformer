...
class DenoiseModel(LightningModule):
    def __init__(self, params, train_loader, val_loader):
        ...
        if params.encoder_name == 'latticeformer':
            self.model = Latticeformer(params, return_pos=True)
        else:
    ...
    def add_noise(self, batch):
        pos = batch.pos
        sigma = 2
        noise = torch.randn(pos.shape) * sigma
        pos_noise = pos + noise
        batch.pos = pos_noise
        print(f'{pos=}, {pos_noise=}')
        return pos, batch
    ...
    def training_step(self, batch, batch_idx):
        ...
        pos, batch = self.add_noise(batch)
        pred_label, pred_pos = self.forward(batch)
